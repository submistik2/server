...
function showLoginPage() {
  document.getElementById('app').innerHTML = `
    ...
    <button id="googleBtn" class="btn btn-primary" data-i18n="signIn"></button>
...
function showFilesPage() {
  document.getElementById('app').innerHTML = `
    ...
    <div id="fileList">
      ...
      <button id="uploadBtn" data-i18n="uploadFile"></button>
    </div>
  `;
  loadUserFiles();
...
async function loadUserFiles() {
  ...
  if (querySnapshot.empty) {
    list.innerHTML = `<p data-i18n="noFiles"></p>
      <input type="file" id="fileInput" multiple>
      <button id="uploadBtn" data-i18n="uploadFile"></button>`;
    document.getElementById('uploadBtn').addEventListener('click', uploadFiles);
  } else {
...
window.logoutUser = () => {
  import('./utils.js').then(utils => utils.logoutUser());
};
